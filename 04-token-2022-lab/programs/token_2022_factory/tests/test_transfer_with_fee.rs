...
#[test]
fn transfer_with_fee_moves_tokens_and_withholds_fee() {
    ...
    let (alice_account, alice_fee_amount) =
        read_token_account_with_fee(&svm, &alice_token_account.pubkey());
    let (bob_account, bob_fee_amount) =
        read_token_account_with_fee(&svm, &bob_token_account.pubkey());
    let mint_state = read_mint(&svm, &mint.pubkey());

    assert_eq!(
        u64::from(alice_account.amount),
        MINT_AMOUNT - TRANSFER_AMOUNT
    );
    assert_eq!(
        u64::from(bob_account.amount),
        TRANSFER_AMOUNT - EXPECTED_FEE
    );
    assert_eq!(u64::from(alice_fee_amount.withheld_amount), 0);
    assert_eq!(u64::from(bob_fee_amount.withheld_amount), EXPECTED_FEE);
    assert_eq!(u64::from(mint_state.supply), MINT_AMOUNT);
}

#[test]
fn wrong_fee_rejects_transfer() {
    let (mut svm, admin) = setup();
    let mint = Keypair::new();
    let alice = Keypair::new();
    let bob = Keypair::new();
