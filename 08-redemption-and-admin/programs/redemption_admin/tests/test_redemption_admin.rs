...
fn protocol_config_pda() -> (Pubkey, u8) {
    Pubkey::find_program_address(&[redemption_admin::PROTOCOL_SEED], &redemption_admin::id())
}

fn redemption_vault_pda(protocol_config: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(
        &[
            redemption_admin::REDEMPTION_VAULT_SEED,
...
fn redemption_request_pda(protocol_config: Pubkey, request_id: u64) -> (Pubkey, u8) {
    Pubkey::find_program_address(
        &[
            redemption_admin::REDEMPTION_REQUEST_SEED,
            protocol_config.as_ref(),
...
fn admin_action_log_pda(protocol_config: Pubkey, action_id: u64) -> (Pubkey, u8) {
    Pubkey::find_program_address(
        &[
            redemption_admin::ADMIN_ACTION_LOG_SEED,
            protocol_config.as_ref(),
...
fn initialize_protocol_ix(
    ...
    Instruction::new_with_bytes(
        redemption_admin::id(),
        &redemption_admin::instruction::InitializeProtocol {}.data(),
        redemption_admin::accounts::InitializeProtocol {
            admin,
            protocol_config,
            redemption_vault,
...
fn request_redemption_ix(
    ...
    Instruction::new_with_bytes(
        redemption_admin::id(),
        &redemption_admin::instruction::RequestRedemption { amount }.data(),
        redemption_admin::accounts::RequestRedemption {
            owner,
            protocol_config,
            redemption_vault,
            redemption_request,
...
fn cancel_redemption_ix(
    ...
    Instruction::new_with_bytes(
        redemption_admin::id(),
        &redemption_admin::instruction::CancelRedemption {}.data(),
        redemption_admin::accounts::CancelRedemption {
            owner,
            protocol_config,
            redemption_vault,
            redemption_request,
...
fn complete_redemption_ix(
    ...
    Instruction::new_with_bytes(
        redemption_admin::id(),
        &redemption_admin::instruction::CompleteRedemption {}.data(),
        redemption_admin::accounts::AdminRedemptionAction {
            admin,
            protocol_config,
            redemption_vault,
            redemption_request,
...
fn reject_redemption_ix(
    ...
    Instruction::new_with_bytes(
        redemption_admin::id(),
        &redemption_admin::instruction::RejectRedemption {}.data(),
        redemption_admin::accounts::AdminRedemptionAction {
            admin,
            protocol_config,
            redemption_vault,
...
fn set_redemptions_paused_ix(
    ...
    Instruction::new_with_bytes(
        redemption_admin::id(),
        &redemption_admin::instruction::SetRedemptionsPaused { paused }.data(),
        redemption_admin::accounts::SetRedemptionsPaused {
            admin,
            protocol_config,
            admin_action_log,
            system_program: system_program::ID,
...
fn begin_admin_transfer_ix(
    ...
    Instruction::new_with_bytes(
        redemption_admin::id(),
        &redemption_admin::instruction::BeginAdminTransfer { new_admin }.data(),
        redemption_admin::accounts::BeginAdminTransfer {
            admin,
            protocol_config,
            admin_action_log,
            system_program: system_program::ID,
...
fn accept_admin_transfer_ix(
    ...
    Instruction::new_with_bytes(
        redemption_admin::id(),
        &redemption_admin::instruction::AcceptAdminTransfer {}.data(),
        redemption_admin::accounts::AcceptAdminTransfer {
            pending_admin,
            protocol_config,
            admin_action_log,
            system_program: system_program::ID,
...
fn create_request(fixture: &mut Fixture, owner: &Keypair, amount: u64) -> Pubkey {
    let state = test_support::deserialize_account::<redemption_admin::ProtocolConfig>(
        &fixture.svm,
...
fn initialize_protocol_creates_config_and_vault() {
    ...
    let config = test_support::deserialize_account::<redemption_admin::ProtocolConfig>(
        &svm,
        &protocol_config,
    );
    let vault = test_support::deserialize_account::<redemption_admin::RedemptionVault>(
        &svm,
        &redemption_vault,
    );
...
fn request_redemption_creates_pending_request_and_updates_vault() {
    ...
    let request_state = test_support::deserialize_account::<redemption_admin::RedemptionRequest>(
        &fixture.svm,
        &request,
    );
    let vault = test_support::deserialize_account::<redemption_admin::RedemptionVault>(
    ...
    assert_eq!(
        ...
        redemption_admin::RedemptionStatus::Pending
    );
...
fn complete_redemption_updates_request_vault_and_admin_log() {
    ...
    let log = test_support::deserialize_account::<redemption_admin::AdminActionLog>(
        &fixture.svm,
        &admin_log,
    );
    ...
    assert_eq!(
        request_state.status,
        redemption_admin::RedemptionStatus::Completed
    );
    assert_eq!(vault.outstanding_amount, 0);
    assert_eq!(vault.total_completed, REDEMPTION_AMOUNT);
    assert_eq!(
        log.action,
        redemption_admin::AdminAction::CompleteRedemption
    );
...
fn reject_redemption_updates_request_vault_and_admin_log() {
    ...
    assert_eq!(
        request_state.status,
        redemption_admin::RedemptionStatus::Rejected
    );
    assert_eq!(vault.outstanding_amount, 0);
