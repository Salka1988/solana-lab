...
fn withdraw_ix(user: Pubkey, vault_config: Pubkey, amount_lamports: u64) -> Instruction {
    Instruction::new_with_bytes(
        vault_authority::id(),
        &vault_authority::instruction::Withdraw { amount_lamports }.data(),
        vault_authority::accounts::Withdraw { user, vault_config }.to_account_metas(None),
    )
}
...
#[test]
fn withdraw_decreases_state_and_returns_lamports() {
    let (mut svm, user) = setup();
    let (vault_config, _) = vault_config_pda(&user.pubkey());

    initialize_and_deposit(&mut svm, &user, vault_config);
    ...
    assert!(send_instruction(
        &mut svm,
        &user,
        withdraw_ix(user.pubkey(), vault_config, WITHDRAW_LAMPORTS)
    ));
    ...
    assert_eq!(
        vault_state.total_deposited_lamports,
        DEPOSIT_LAMPORTS - WITHDRAW_LAMPORTS
    );
    assert_eq!(
        vault_lamports_after,
        vault_lamports_before - WITHDRAW_LAMPORTS
    );
    assert!(user_lamports_after > user_lamports_before);
}

#[test]
fn withdraw_rejects_amount_over_deposited_balance() {
    let (mut svm, user) = setup();
    let (vault_config, _) = vault_config_pda(&user.pubkey());
    ...
    assert!(!send_instruction(
        &mut svm,
        &user,
        withdraw_ix(user.pubkey(), vault_config, DEPOSIT_LAMPORTS + 1)
    ));
...
#[test]
fn withdraw_rejects_wrong_pda_substitution() {
    let (mut svm, user) = setup();
    ...
    initialize_and_deposit(&mut svm, &user, vault_config);
    initialize_and_deposit(&mut svm, &other_user, other_vault_config);

    assert!(!send_instruction(
        &mut svm,
        &user,
        withdraw_ix(user.pubkey(), other_vault_config, WITHDRAW_LAMPORTS)
    ));
    ...
    assert_eq!(vault_state.total_deposited_lamports, DEPOSIT_LAMPORTS);
    assert_eq!(other_vault_state.total_deposited_lamports, DEPOSIT_LAMPORTS);
}
