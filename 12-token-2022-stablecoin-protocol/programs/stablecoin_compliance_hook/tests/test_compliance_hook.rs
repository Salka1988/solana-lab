...
fn set_transfer_limits_ix(
    ...
    Instruction::new_with_bytes(
        stablecoin_compliance_hook::id(),
        &stablecoin_compliance_hook::instruction::SetTransferLimits {
            max_transfer_amount,
            daily_transfer_limit,
        }
        .data(),
        stablecoin_compliance_hook::accounts::SetTransferLimits { admin, config }
            .to_account_metas(None),
    )
}
...
fn set_protocol_paused_ix(admin: Pubkey, config: Pubkey, paused: bool) -> Instruction {
    Instruction::new_with_bytes(
        stablecoin_compliance_hook::id(),
        &stablecoin_compliance_hook::instruction::SetProtocolPaused { paused }.data(),
        stablecoin_compliance_hook::accounts::SetProtocolPaused { admin, config }
            .to_account_metas(None),
    )
}
...
fn initialize_compliance_config_creates_expected_state() {
    ...
    let state = test_support::deserialize_account::<stablecoin_compliance_hook::ComplianceConfig>(
        &svm, &config,
    );
...
fn initialize_extra_account_meta_list_creates_validation_account() {
    ...
    assert_eq!(account.owner, stablecoin_compliance_hook::id());
    assert_eq!(
        account.data.len(),
        stablecoin_compliance_hook::EXTRA_ACCOUNT_META_LIST_SPACE
    );
}
...
fn execute_allows_allowlisted_transfer_and_updates_daily_amount() {
    ...
    let source_state = test_support::deserialize_account::<
        stablecoin_compliance_hook::UserCompliance,
    >(&svm, &fixture.source_compliance);
