...
fn create_stablecoin_mint_initializes_token_2022_mint_config_and_supply_stats() {
    ...
    let protocol_state = test_support::deserialize_account::<stablecoin_issuer::ProtocolConfig>(
        &svm,
        &protocol_config,
    );
    let mint_config_state = test_support::deserialize_account::<
        stablecoin_issuer::StablecoinMintConfig,
    >(&svm, &mint_config);
    let supply_stats_state = test_support::deserialize_account::<
        stablecoin_issuer::GlobalSupplyStats,
    >(&svm, &supply_stats);
